...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"TPProgressHUD needs to be accessed on the main thread.");

CGFloat const TPProgressMaxOffset = 1000000.f;

static const CGFloat TPDefaultPadding = 4.f;
static const CGFloat TPDefaultLabelFontSize = 16.f;
static const CGFloat TPDefaultDetailsLabelFontSize = 12.f;
...
@interface TPProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    TPProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    TPProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (TPProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            TPProgressHUD *hud = (TPProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = TPProgressHUDAnimationFade;
    _mode = TPProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(TPProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == TPProgressHUDAnimationZoom) {
        type = animatingIn ? TPProgressHUDAnimationZoomIn : TPProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == TPProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == TPProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == TPProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == TPProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    TPProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<TPProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    TPBackgroundView *backgroundView = [[TPBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = TPProgressHUDBackgroundStyleSolidColor;
    ...
    TPBackgroundView *bezelView = [TPBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:TPDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:TPDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [TPProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:TPDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[TPRoundProgressView class]];

    TPProgressHUDMode mode = self.mode;
    if (mode == TPProgressHUDModeIndeterminate) {
    ...
    else if (mode == TPProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[TPBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == TPProgressHUDModeDeterminate || mode == TPProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[TPRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == TPProgressHUDModeAnnularDeterminate) {
            [(TPRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == TPProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == TPProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[TPProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[TPProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[TPRoundProgressView class]]) {
        TPRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TPRoundProgressView appearanceWhenContainedIn:[TPProgressHUD class], nil];
#else
        appearance = [TPRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[TPProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((TPRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((TPRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[TPBarProgressView class]]) {
        TPBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TPBarProgressView appearanceWhenContainedIn:[TPProgressHUD class], nil];
#else
        appearance = [TPBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[TPProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((TPBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((TPBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    TPBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? TPDefaultPadding : 0.f;
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == TPProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation TPRoundProgressView
...
@implementation TPBarProgressView
...
@implementation TPBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = TPProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(TPProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    TPProgressHUDBackgroundStyle style = self.style;
    if (style == TPProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == TPProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation TPProgressHUDRoundedButton
